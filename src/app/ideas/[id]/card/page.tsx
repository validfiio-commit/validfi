...
interface CardData {
  ...
  summary: string; founder_onchain?: string; validatedAt: string;
  walletStats?: {
    portfolioUsd: string; chains: number; txCount: number;
    defiPositions: number; stakedUsd: string; pnl30d: string;
  } | null;
}
      ...
      >
        <div className="relative rounded-[24px] overflow-hidden" style={{
          ...
          <div className="relative z-10 p-9 pb-7">
            ...
            {data.walletStats && parseFloat(data.walletStats.portfolioUsd) > 0 && (
              <div className="flex items-center gap-3 mb-5 py-3 px-4 rounded-xl" style={{
                background: "linear-gradient(135deg, rgba(168,85,247,0.04) 0%, rgba(0,240,255,0.03) 100%)",
                border: "1px solid rgba(168,85,247,0.08)",
              }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2" style={{ flexShrink: 0 }}><path d="M21 12V7H5a2 2 0 010-4h14v4"/><path d="M3 5v14a2 2 0 002 2h16v-5"/></svg>
                <div className="flex flex-wrap gap-x-4 gap-y-1">
                  <span className="text-[10px]" style={{ fontFamily: "Space Mono, monospace", color: "rgba(168,85,247,0.7)" }}>
                    Portfolio: <span style={{ color: "#a855f7" }}>${parseFloat(data.walletStats.portfolioUsd) >= 1000 ? (parseFloat(data.walletStats.portfolioUsd)/1000).toFixed(1) + "K" : parseFloat(data.walletStats.portfolioUsd).toFixed(0)}</span>
                  </span>
                  {data.walletStats.txCount > 0 && (
                    <span className="text-[10px]" style={{ fontFamily: "Space Mono, monospace", color: "rgba(0,240,255,0.5)" }}>
                      {data.walletStats.txCount.toLocaleString()} txns
                    </span>
                  )}
                  {data.walletStats.chains > 0 && (
                    <span className="text-[10px]" style={{ fontFamily: "Space Mono, monospace", color: "rgba(0,240,255,0.5)" }}>
                      {data.walletStats.chains} chains
                    </span>
                  )}
                  {data.walletStats.defiPositions > 0 && (
                    <span className="text-[10px]" style={{ fontFamily: "Space Mono, monospace", color: "rgba(74,222,128,0.6)" }}>
                      {data.walletStats.defiPositions} DeFi
                    </span>
                  )}
                  {parseFloat(data.walletStats.stakedUsd) > 0 && (
                    <span className="text-[10px]" style={{ fontFamily: "Space Mono, monospace", color: "rgba(74,222,128,0.6)" }}>
                      ${parseFloat(data.walletStats.stakedUsd) >= 1000 ? (parseFloat(data.walletStats.stakedUsd)/1000).toFixed(1) + "K" : parseFloat(data.walletStats.stakedUsd).toFixed(0)} staked
                    </span>
                  )}
                </div>
                <span className="text-[7px] tracking-[1.5px] uppercase ml-auto shrink-0" style={{ fontFamily: "Space Mono, monospace", color: "rgba(238,234,226,0.12)" }}>ON-CHAIN</span>
              </div>
            )}
